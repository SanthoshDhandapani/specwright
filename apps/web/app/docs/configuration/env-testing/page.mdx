...
| Field | Default | Description |
|---|---|---|
| `ENABLE_COVERAGE` | `false` | Capture V8 code coverage during the test run. Automatically set to `true` by `pnpm test:bdd:coverage`. Set globally if you want coverage on every `pnpm test:bdd` run. |
