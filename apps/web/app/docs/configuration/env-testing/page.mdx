...
# PIPELINE_TICKET_ID=PROJ-123    # required when org hooks enforce ticket IDs
...
| `AUTH_BASE_URL` | — | Optional. Set when real signin only works on a different host than `BASE_URL` (e.g. you run tests against `localhost` but auth requires a hosted environment). Enables [split-auth](/docs/configuration/split-auth) — signin happens at `AUTH_BASE_URL`, then the captured storage state is rewritten so tokens land on `BASE_URL`. |
...
| `PIPELINE_TICKET_ID` | — | Jira or issue ticket ID (e.g. `PROJ-123`) prepended to pipeline run names. Required when org-level hooks enforce ticket IDs on all pipeline runs. |
...
| Field | Default | Description |
|---|---|---|
| `ENABLE_COVERAGE` | `false` | Capture V8 code coverage during the test run. Automatically set to `true` by `pnpm test:e2e:coverage`. Set globally if you want coverage on every `pnpm test:bdd` run. |
| `COVERAGE_EXCLUDE` | (empty) | Comma-separated glob/substring patterns to exclude from coverage measurement (a shell env var overrides this value). Patterns without wildcards match as substrings; `*` matches one path segment, `**` across segments. Example: `src/api/,.stories.,/__mocks__/,**/*.generated.js`. Built-in defaults already exclude `node_modules`, `*.test.*`, `*.spec.*`, `*.stories.*`, CSS/asset modules, and external hosts. |
| `COVERAGE_SOURCE_ROOTS` | `src` | Comma-separated source root directories used to reconstruct full file paths in the report (when build tools emit bare basenames). Examples: `app` for Next.js App Router, `src,lib` for multi-root layouts, `packages/web/src` for monorepos. Walker indexes `.ts/.tsx/.js/.jsx/.mjs/.cjs/.vue/.svelte/.astro`. |
| `COVERAGE_ALLOW_REMOTE` | `false` | Override the localhost-only guard. Set to `true` only if your deployed build ships source maps. Default behaviour refuses to run against non-localhost URLs because deployed bundles usually strip source maps, producing useless reports. |

See [Code Coverage reference](/docs/reference/code-coverage) for full details on output format, LCOV integration, and CI setup.
...
Specwright is built on [Playwright](https://playwright.dev) and [playwright-bdd](https://vitalets.github.io/playwright-bdd/). Any environment variable supported by either library also works in `.env.testing`.
...
> For the full list of Playwright environment variables, see the [Playwright configuration docs](https://playwright.dev/docs/test-configuration) and [playwright-bdd configuration](https://vitalets.github.io/playwright-bdd/configuration/).
