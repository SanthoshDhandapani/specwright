...
| `COVERAGE_EXCLUDE` | (empty) | Comma-separated substring patterns to exclude from coverage measurement. Example: `src/api/,src/store/,.stories.,/__mocks__/`. Built-in defaults already exclude `node_modules`, `*.test.*`, `*.spec.*`, `.d.ts`, `*.css`, and external hosts. |
