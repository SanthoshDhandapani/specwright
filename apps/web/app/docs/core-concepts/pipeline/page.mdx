...
Phase 2   Detection & Routing   Detect input sources (Jira / file / text)
Phase 3   Input Processing      Fetch Jira / convert file / normalise text → parsed plan
Phase 4   Exploration           Browser agent explores live app → seed file + plan
Phase 5   Seed Validation       (optional) Run seed.spec.js to verify selectors
Phase 6   ⛔ User Approval       Present plan — wait for explicit yes before proceeding
Phase 7   BDD Generation        bdd-generator + code-generator → .feature + steps.js
Phase 8   Test & Heal           (optional) Run tests, auto-fix failures up to 3 iterations
Phase 9   Cleanup               Remove intermediate plan/seed files
