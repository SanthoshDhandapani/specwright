...
3. Fill email and password from the `e2e_explore` auth data block using `browser_fill` / `browser_type`
...
2. Fill credentials from the `e2e_explore` auth data block
...
- **Browser tools unavailable:** `@playwright/mcp` not installed in `@specwright/mcp` — check server logs. ⛔ Do NOT fall back to reading source files or grepping the codebase.
- **Browser profile locked / navigation fails:** Report the error to the user and ask them to restart Claude Desktop. ⛔ Do NOT attempt to read source code as a workaround.
...
- **Page load timeout:** Retry `browser_navigate` once, then report to user
